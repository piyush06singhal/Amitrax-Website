...
import { ArrowRight, Sparkles, CheckCircle2, Layers, Cpu, ExternalLink } from 'lucide-react';
...
export const FeaturedProductExperience: React.FC<FeaturedProductExperienceProps> = ({ product }) => {
  ...
  return (
    <div
      ...
      className="relative w-full rounded-3xl bg-gradient-to-br from-[#0a1026] via-[#070c1d] to-[#04060f] border border-cyan-500/30 overflow-hidden shadow-2xl p-6 sm:p-8 lg:p-12"
    >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 font-mono text-xs font-semibold">
          ...
          <div className="space-y-3">
            <h3 className="text-3xl sm:text-4xl lg:text-5xl font-bold font-display text-white tracking-tight">
              {product.name}
            </h3>
          ...
          <div className="space-y-2.5 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {product.keyCapabilities.slice(0, 4).map((cap, idx) => (
                <div
                  key={idx}
                  className="p-3 rounded-xl bg-white/[0.03] border border-white/5 flex items-start gap-2.5 text-xs text-slate-200"
                >
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {product.technologies.slice(0, 4).map((tech, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/10 text-xs font-mono text-slate-200 flex items-center gap-2"
                  title={tech.role}
                >
